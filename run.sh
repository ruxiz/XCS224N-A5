...
if [ "$1" = "train" ]; then
    ...
    CUDA_VISIBLE_DEVICES=0 python3 run.py decode model.bin ./en_es_data/test.es outputs/test_outputs.txt --cuda

elif [ "$1" = "train_local" ]; then
  	python3 run.py train --train-src=./en_es_data/train.es --train-tgt=./en_es_data/train.en \
        --dev-src=./en_es_data/dev.es --dev-tgt=./en_es_data/dev.en --vocab=vocab.json --batch-size=2

elif [ "$1" = "test_local" ]; then
    mkdir -p outputs
    touch outputs/test_outputs.txt
    python3 run.py decode model.bin ./en_es_data/grader.es outputs/test_outputs.txt
